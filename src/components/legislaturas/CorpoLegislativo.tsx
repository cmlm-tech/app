...
interface CorpoLegislativoProps {
  ...
  onEdit?: (vereador: VereadorComCondicao) => void;
  liderancasMap?: Record<number, 'governo' | 'oposicao'>;
}

export function CorpoLegislativo({ vereadores, isAdmin, onAdicionarClick, onRemove, onEdit, liderancasMap = {} }: CorpoLegislativoProps) {
  return (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4">
              {vereadores.map(vereador => (
                <CardVereador
                  ...
                  vereador={vereador}
                  isAdmin={isAdmin}
                  onRemove={() => onRemove(vereador)}
                  onEdit={onEdit ? () => onEdit(vereador) : undefined}
